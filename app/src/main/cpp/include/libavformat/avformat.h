 ...
 * AVFormatContext, they can be examined from a user m_Program by calling
 ...
 * consumed). The calling m_Program can handle such unrecognized options as it
 * wishes, e.g.
 * @code
 * AVDictionaryEntry *e;
...
typedef struct AVProgram {
    int            id;
    int            flags;
    enum AVDiscard discard;        ///< selects which m_Program to discard and which to feed to the caller
 ...
 *
 * @param ic    media file handle
 * @param last  the last found m_Program, the search will start after this
 *              m_Program, or from the beginning if it is NULL
 * @param s     stream index
 * @return the next m_Program which belongs to s, NULL if no m_Program is found or
 *         the last m_Program is not among the programs of ic.
 */
 ...
 *                          m_Program) to this one, or -1 if none
